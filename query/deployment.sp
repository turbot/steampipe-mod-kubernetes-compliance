...
query "deployment_container_argument_service_account_key_file_appropriate" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
...
query "deployment_container_streaming_connection_idle_timeout_not_zero" {
  sql = <<-EOQ
    select
      distinct(coalesce(uid, concat(path, ':', start_line))) as resource,
      case
...
query "deployment_container_strong_kubelet_cryptographic_ciphers" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
...
query "deployment_container_argument_rotate_kubelet_server_certificate_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "deployment_container_argument_service_account_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "deployment_container_strong_kube_apiserver_cryptographic_ciphers" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
...
query "deployment_container_argument_request_timeout_appropriate" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
        trim('"' from split_part(co::text, '=', 2)) as value,
        p.name as deployment
      from
      ...
      select
        p.name as deployment_name,
        p.uid as deployment_uid,
        p.path as path,
    ...
    select
      coalesce(p.deployment_uid, concat(p.path, ':', p.start_line)) as resource,
      case
      ...
      p.deployment_name as deployment_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
    from
...
query "deployment_container_secrets_defined_as_files" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
