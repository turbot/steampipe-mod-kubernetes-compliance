...
query "pod_container_argument_kube_scheduler_bind_address_127_0_0_1" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        ...
        when l.container_name is not null and (p.value -> 'command') @> '["kube-scheduler"]' and ((l.value) like '%127.0.0.1%') then p.value ->> 'name' || ' kube-scheduler bind address set to 127.0.0.1.'
        else p.value ->> 'name' || ' kube-scheduler bind address not set to 127.0.0.1.'
      end as reason,
      p.pod_name as pod_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "pod_container_argument_security_context_deny_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "pod_container_argument_secure_port_not_0" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "pod_container_kubernetes_dashboard_not_deployed" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "pod_container_streaming_connection_idle_timeout_not_zero" {
  sql = <<-EOQ
    select
      distinct(coalesce(uid, concat(path, ':', start_line))) as resource,
      case
...
query "pod_container_strong_kubelet_cryptographic_ciphers" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        when (p.value -> 'command') is null or not ((p.value -> 'command') @> '["kubelet"]') then 'ok'
        when l.container_name is not null and (p.value -> 'command') @> '["kubelet"]'
      ...
      case
        ...
        when l.container_name is not null and (p.value -> 'command') @> '["kubelet"]'
          and string_to_array(l.value, ',') <@ array['TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_128_GCM_SHA256']
        then p.value ->> 'name' || ' kubelet uses strong cryptographic ciphers.'
        else p.value ->> 'name' || ' kubelet not using strong cryptographic ciphers.'
      end as reason,
      p.pod_name as pod_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "pod_container_argument_rotate_kubelet_server_certificate_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "pod_container_argument_etcd_peer_certfile_and_peer_keyfile_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "pod_container_argument_kube_controller_manager_bind_address_127_0_0_1" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        when (p.value -> 'command') is null or not ((p.value -> 'command') @> '["kube-controller-manager"]') then 'ok'
        when l.container_name is not null and (p.value -> 'command') @> '["kube-controller-manager"]' and ((l.value) like '%127.0.0.1%') then 'ok'
        else 'alarm'
      end as status,
      case
        when (p.value -> 'command') is null then p.value ->> 'name' || ' command not defined.'
        when not ((p.value -> 'command') @> '["kube-controller-manager"]') then p.value ->> 'name' || ' kube-controller-manager not defined.'
        when l.container_name is not null and (p.value -> 'command') @> '["kube-controller-manager"]' and ((l.value) like '%127.0.0.1%') then p.value ->> 'name' || ' kube-controller-manager bind address set to 127.0.0.1.'
        else p.value ->> 'name' || ' kube-controller-manager bind address not set to 127.0.0.1.'
      end as reason,
      p.pod_name as pod_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "pod_container_argument_service_account_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "pod_container_strong_kube_apiserver_cryptographic_ciphers" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        when (p.value -> 'command') is null or not ((p.value -> 'command') @> '["kube-apiserver"]') then 'ok'
        when l.container_name is not null and (p.value -> 'command') @> '["kube-apiserver"]'
          and string_to_array(l.value, ',') <@ array['TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_128_GCM_SHA256']
        then 'ok'
        else 'alarm'
      end as status,
      case
        ...
        when l.container_name is not null and (p.value -> 'command') @> '["kube-apiserver"]'
          and string_to_array(l.value, ',') <@ array['TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_128_GCM_SHA256']
        then p.value ->> 'name' || ' kube-apiserver uses strong cryptographic ciphers.'
        else p.value ->> 'name' || ' kube-apiserver not using strong cryptographic ciphers.'
      end as reason,
      p.pod_name as pod_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
