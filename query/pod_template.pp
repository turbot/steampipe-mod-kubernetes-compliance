...
query "pod_template_container_host_port_not_specified" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "pod_template_container_argument_request_timeout_appropriate" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
        trim('"' from split_part(co::text, '=', 2)) as value,
        p.name as pod_template
      from
        kubernetes_pod_template as p,
        jsonb_array_elements(template -> 'spec' -> 'containers') as c,
        jsonb_array_elements_text(c -> 'command') as co
      ...
      select
        p.name as pod_template_name,
        p.uid as pod_template_uid,
        p.path as path,
    ...
    select
      coalesce(p.pod_template_uid, concat(p.path, ':', p.start_line)) as resource,
      case
      ...
      p.pod_template_name as pod_template_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
    from
...
query "pod_template_container_secrets_defined_as_files" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
