...
query "statefulset_host_network_access_disabled" {
  sql = <<-EOQ
    select
      ...
      case
        when (template -> 'spec' ->> 'hostNetwork')::bool then 'StatefulSet pods using host network.'
...
query "statefulset_hostpid_hostipc_sharing_disabled" {
  sql = <<-EOQ
    select
      ...
      case
        when (template -> 'spec' ->> 'hostPID')::bool then 'StatefulSet pods share host PID namespaces.'
        when (template -> 'spec' ->> 'hostIPC')::bool then 'StatefulSet pods share host IPC namespaces.'
...
query "statefulset_container_argument_kube_apiserver_etcd_certfile_and_keyfile_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_kube_scheduler_profiling_disabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_kube_scheduler_bind_address_127_0_0_1" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      coalesce(s.statefulset_uid, concat(s.path, ':', s.start_line)) as resource,
      case
        when (s.value -> 'command') is null or not ((s.value -> 'command') @> '["kube-scheduler"]') then 'ok'
        when l.container_name is not null and (s.value -> 'command') @> '["kube-scheduler"]' and ((l.value) like '%127.0.0.1%') then 'ok'
        else 'alarm'
      end as status,
      case
        when (s.value -> 'command') is null then s.value ->> 'name' || ' command not defined.'
        when not ((s.value -> 'command') @> '["kube-scheduler"]') then s.value ->> 'name' || ' kube-scheduler not defined.'
        when l.container_name is not null and (s.value -> 'command') @> '["kube-scheduler"]' and ((l.value) like '%127.0.0.1%') then s.value ->> 'name' || ' kube-scheduler bind address set to 127.0.0.1.'
        else s.value ->> 'name' || ' kube-scheduler bind address not set to 127.0.0.1.'
      end as reason,
      s.statefulset_name as statefulset_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "statefulset_container_argument_protect_kernel_defaults_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_make_iptables_util_chains_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_kubelet_tls_cert_file_and_tls_private_key_file_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_no_argument_hostname_override_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_kube_controller_manager_profiling_disabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_etcd_auto_tls_disabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_token_auth_file_not_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_kubelet_certificate_authority_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_kube_apiserver_profiling_disabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_secure_port_not_0" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_service_account_key_file_appropriate" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        when (s.value -> 'command') is null or not ((s.value -> 'command') @> '["kube-apiserver"]') then 'ok'
        when l.container_name is not null and (s.value -> 'command') @> '["kube-apiserver"]' and ((l.value) like '%.pem') then 'ok'
        else 'alarm'
      end as status,
      case
        when (s.value -> 'command') is null then s.value ->> 'name' || ' command not defined.'
        when not ((s.value -> 'command') @> '["kube-apiserver"]') then s.value ->> 'name' || ' kube-apiserver not defined.'
        when l.container_name is not null and (s.value -> 'command') @> '["kube-apiserver"]' and ((l.value) like '%.pem') then s.value ->> 'name' || ' service-account-key-file set appropriate.'
        else s.value ->> 'name' || ' service-account-key-file set inappropriate.'
      end as reason,
      s.statefulset_name as statefulset_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "statefulset_container_kubernetes_dashboard_not_deployed" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_streaming_connection_idle_timeout_not_zero" {
  sql = <<-EOQ
    select
      distinct(coalesce(uid, concat(path, ':', start_line))) as resource,
      case
...
query "statefulset_container_strong_kubelet_cryptographic_ciphers" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        when (s.value -> 'command') is null or not ((s.value -> 'command') @> '["kubelet"]') then 'ok'
        when l.container_name is not null and (s.value -> 'command') @> '["kubelet"]'
      ...
      case
        ...
        when l.container_name is not null and (s.value -> 'command') @> '["kubelet"]'
          and string_to_array(l.value, ',') <@ array['TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_128_GCM_SHA256']
        then s.value ->> 'name' || ' kubelet uses strong cryptographic ciphers.'
        else s.value ->> 'name' || ' kubelet not using strong cryptographic ciphers.'
      end as reason,
      s.statefulset_name as statefulset_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "statefulset_container_argument_rotate_kubelet_server_certificate_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_etcd_peer_certfile_and_peer_keyfile_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_service_account_enabled" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_argument_kubelet_client_ca_file_configured" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        when (s.value -> 'command') is null or not ((s.value -> 'command') @> '["kubelet"]') then 'ok'
        when l.container_name is not null and (s.value -> 'command') @> '["kubelet"]' and l.value is not null and l.value <> '' then 'ok'
        else 'alarm'
      end as status,
      case
        when (s.value -> 'command') is null then s.value ->> 'name' || ' command not defined.'
        when not ((s.value -> 'command') @> '["kubelet"]') then s.value ->> 'name' || ' kubelet not defined.'
        when l.container_name is not null and (s.value -> 'command') @> '["kubelet"]' and l.value is not null and l.value <> '' then s.value ->> 'name' || ' kubelet client CA file configured.'
        else s.value ->> 'name' || ' kubelet client CA file is not configured.'
      end as reason,
      s.statefulset_name as statefulset_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "statefulset_container_argument_kubelet_terminated_pod_gc_threshold_configured" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
...
query "statefulset_container_argument_kube_apiserver_tls_cert_file_and_tls_private_key_file_configured" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_strong_kube_apiserver_cryptographic_ciphers" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        when (s.value -> 'command') is null or not ((s.value -> 'command') @> '["kube-apiserver"]') then 'ok'
        when l.container_name is not null and (s.value -> 'command') @> '["kube-apiserver"]'
          and string_to_array(l.value, ',') <@ array['TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_128_GCM_SHA256']
        then 'ok'
        else 'alarm'
      end as status,
      case
        ...
        when l.container_name is not null and (s.value -> 'command') @> '["kube-apiserver"]'
          and string_to_array(l.value, ',') <@ array['TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305','TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_256_GCM_SHA384','TLS_RSA_WITH_AES_128_GCM_SHA256']
        then s.value ->> 'name' || ' kube-apiserver uses strong cryptographic ciphers.'
        else s.value ->> 'name' || ' kube-apiserver not using strong cryptographic ciphers.'
      end as reason,
      s.statefulset_name as statefulset_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
...
query "statefulset_container_host_port_not_specified" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
...
query "statefulset_container_secrets_defined_as_files" {
  sql = <<-EOQ
    select
      coalesce(uid, concat(path, ':', start_line)) as resource,
      case
