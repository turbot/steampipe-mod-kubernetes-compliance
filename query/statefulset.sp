...
query "statefulset_container_argument_bind_address_127_0_0_1" {
  sql = <<-EOQ
    with container_list as (
      select
        c ->> 'name' as container_name,
    ...
    select
      ...
      case
        ...
        when l.container_name is not null and (s.value -> 'command') @> '["kube-scheduler"]' and ((l.value) like '%127.0.0.1%') then s.value ->> 'name' || ' bind address set to 127.0.0.1.'
        else s.value ->> 'name' || ' bind address not set to 127.0.0.1.'
      end as reason,
      s.statefulset_name as statefulset_name
      ${local.tag_dimensions_sql}
      ${local.common_dimensions_sql}
